...
module.exports = class voiceStateUpdate {
  ...
  async run(oldState, newState) {
    ...
    const doc_user = await this.client.database.users.findOne({ idU: user.id });
    const doc_guild = await this.client.database.guilds.findOne({ idS: guild.id });
    const call = doc_user?.infoCall;
    const call2 = doc_guild?.infoCall;

    const channel = guild.channels.cache.get(doc_guild.logs.channel);
